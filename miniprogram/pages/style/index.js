Page({
  ...
  generateSuggestion(answers) {
    ...
    const { currentDateStr, currentSeasonEn, currentSeasonZh, weatherText, weatherTemp, cityName } = global;
...
Current weather: ${weatherText || 'unknown'}，Temperature: ${weatherTemp || 'unknown'}
Location: ${cityName || 'unknown'}

Based on the following user basic information, current weather condition, temperature, location and the specific season, generate personalized outfit suggestions in Chinese, 150-200 words, using a friendly, feminine and encouraging tone. Remember to praise the user's look from the beginning. 
- Make sure the suggestions are appropriate for the current weather and temperature
- Consider the regional culture and climate when making suggestions
- Base suggestions on the season
- Include specific types of clothing, cuts, color combinations, and material recommendations
- Suggest suitable occasions (commuting/casual/dating/sports)
- Use emoji and line breaks to be more like human conversation
- Do not include any thinking or reasoning process - only provide the final suggestion
